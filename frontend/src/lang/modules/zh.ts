...
const message = {
    commons: {
        ...
        login: {
            ...
            title: '终端管理面板',
            licenseHelper:
                '同意 &laquo; <a href="https://www.fit2cloud.com/legal/licenses.html" target="_blank"> 软件许可协议</a> &raquo;',
    ...
    database: {
        ...
        pgSuperUser: '超级用户',
        loadFromRemoteHelper: '此操作将同步服务器上数据库信息到 系统，是否继续？',
    ...
    container: {
        ...
        composeHelper: '通过系统编辑或者模版创建的编排，将保存在 {0}/docker/compose 路径下',
        deleteFile: '删除文件',
    ...
    cronjob: {
        ...
        cutWebsiteLogHelper: '切割的日志文件会备份到  backup 目录下',
    ...
    setting: {
        ...
        ignoreRule: '排除规则',
        ignoreHelper: '快照时将使用该规则对 系统 数据目录进行压缩备份，请谨慎修改。',
        ...
        recoverHelper1: '恢复需要重启 Docker 以及 系统服务',
        recoverHelper2: '请确保服务器磁盘空间充足 ( 快照文件大小: {0}, 可用空间: {1} )',
        recoverHelper3: '请确保服务器架构与创建快照服务器架构信息保持一致 (当前服务器架构: {0} )',
        rollback: '回滚',
        rollbackHelper:
            '即将回滚本次恢复，回滚将替换所有本次恢复的文件，过程中可能需要重启 Docker 以及当前服务，是否继续？',

        upgrading: '正在升级中，请稍候...',
        upgradeHelper: '升级操作需要重启系统服务，是否继续？',
        noUpgrade: '当前已经是最新版本',
        ...
        unBindDomain: '域名解绑',
        panelSSL: '面板 SSL',
        panelSSLHelper: '面板 SSL 自动续期后需要手动重启系统服务才可生效',
        unBindDomainHelper: '解除域名绑定可能造成系统不安全，是否继续？',
        bindDomainHelper: '设置域名绑定后，仅能通过设置中域名访问当前服务',
